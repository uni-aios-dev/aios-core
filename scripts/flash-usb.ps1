...
#
# Usage:
#   powershell -ExecutionPolicy Bypass -File scripts\flash-usb.ps1 [-Image <path>] [-DiskNumber <n>] [-Force]
#
# Examples:
#   powershell -ExecutionPolicy Bypass -File scripts\flash-usb.ps1 -Force
#   powershell -ExecutionPolicy Bypass -File scripts\flash-usb.ps1 -Image out\aios-kernel-usb.img -DiskNumber 3 -Force
...
# resolve image and default target disk before elevation (needed for relaunch args)
$Image = if ($Image) { $Image } else { $defaultImage }
$disks = Get-Disk | Where-Object { $_.BusType -eq 'USB' -and $_.PartitionStyle -ne 'RAW' }
if (-not $disks) { $disks = Get-Disk | Where-Object { $_.IsRemovable -or $_.BusType -eq 'USB' } }
if (-not $disks) { Write-Err 'No removable/USB disk found.'; exit 1 }
$defDisk = ($disks | Sort-Object Number | Select-Object -First 1).Number
$targetNum = if ($DiskNumber) { $DiskNumber } else { $defDisk }
...
if (-not $principal.IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) {
    ...
    Start-Process powershell.exe -Verb RunAs -ArgumentList "-ExecutionPolicy Bypass -File `"$PSCommandPath`" -Image `"$Image`" -DiskNumber $targetNum -Force"
    exit 0
}
...
$written = 0; $sw = [Diagnostics.Stopwatch]::StartNew()
try {
    ...
    $out.Flush(); $out.Close(); $stream.Close(); $sw.Stop()
}
