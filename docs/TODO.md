...
- [ ] **Milestone 6** (next): native input + storage — PCI enumeration, USB-HID (xHCI) keyboard/mouse replacing PS/2, AHCI/NVMe block driver, and a `crt` module for explicit `memcpy`/`memset`/`memmove`/`memcmp`.
