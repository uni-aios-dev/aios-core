...
- [ ] **Bare-metal USB verification on real hardware (v2.38.2+)**: after flashing Kingston with the latest kernel, verify the TUI renders and keyboard input works on the MSI laptop. If it halts, use the `STEP` markers + `HALT_REASON` + debug port `0x80` to identify the failure point. Fix the root cause (likely PIT IDT gate corruption on real hardware).
...
- [x] **Phase 46: aios-autohal — Hardware Auto-Provisioning & Driver Store (Master Brief)**
  ...
  - [x] Native push-based hot-plug: `native.rs` — `NETLINK_KOBJECT_UEVENT` udev listener on Linux and `RegisterDeviceNotificationW` hidden-window PnP monitor on Windows push a coarse `NativeEvent` (`BusHint`-classified) to `HotplugMonitor`, which triggers an immediate full re-detection; `HotplugConfig::native_enabled` (default on) falls back to the cheap signal + `poll_ms` net (v2.25.0)
...
- [x] **Milestone 5** (v2.34.0): Limine + GOP boot — hybrid BIOS+UEFI ISO, Limine-protocol ELF (`_start`, `.requests` sections, `linker.ld`), HHDM paging + memory map from Limine, native framebuffer text console, `aios-kernel-run` ISO builder + OVMF/legacy QEMU launcher; verified under UEFI GOP and legacy BIOS with a screendump pixel histogram.
- [x] **Milestone 6, part 1** (v2.35.0): PCI enumeration (`pci.rs`, `0xCF8`/`0xCFC`, `PciDevice` records + class names) and the `crt` module (`memcpy`/`memmove`/`memset`/`memcmp`/`bcmp`); `port.rs` gained `outl`/`inl`. Verified in QEMU (6 devices on the default `pc` machine).
