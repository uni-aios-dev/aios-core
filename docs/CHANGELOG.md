...
**Limine** (the same bootloader the Live ISO already uses), so `aios-kernel` boots as a
Limine-protocol ELF from a hybrid **BIOS + UEFI** ISO. The kernel no longer drives a VGA
text buffer: it receives the firmware's **GOP/VBE linear framebuffer** and renders through a
native in-kernel text console (no ratatui/egui — they are `std` libraries).

### Changed
- **`aios-kernel` now targets the Limine boot protocol.** `main.rs` declares the request
  statics in `.requests_start`/`.requests`/`.requests_end` (`BaseRevision`, `EntryPoint(_start)`,
  `StackSize(64 KiB)`, `Hhdm`, `Memmap`, `Rsdp`, `Framebuffer`) and exposes
  `#[no_mangle] pub unsafe extern "C" fn _start() -> !` — the Limine entry point receives the
  HHDM offset, RSDP and framebuffer response.
- **`aios-kernel/linker.ld`** (new) — the Limine higher-half script (`OUTPUT_FORMAT(elf64-x86-64)`,
  base `0xffffffff80000000`, `ENTRY(_start)`, `KEEP` of the three request sections); linked via
  the new **`aios-kernel/.cargo/config.toml`** (`-Tlinker.ld`, `--gc-sections`,
  `codegen-units=1`). `limine = "=0.6.5"` added; `bootloader_api` removed.
- **Native framebuffer console.** `framebuffer.rs` (new) wraps the Limine framebuffer
  (`put_pixel`/`fill_rect`/`clear`/`scroll_up`/`read_pixel`, 2/3/4 bytes-per-pixel via the
  channel masks); `console.rs` (new) is a spin-locked text console on top of it, keeping the
  `vprintln!` macro name; `vga.rs` was **deleted** and `syscalls.rs` now writes through
  `console::write_bytes`.
- **Memory map comes from Limine.** `memory.rs` gained `pub struct MemRegion { start, end }`;
  `init(physical_offset, &[MemRegion])` builds the bump frame allocator from the
  `MEMMAP_USABLE` regions; `physical_offset` is `hhdm.offset`. `heap::HEAP_START` and
  `memory::SELFTEST_ADDR` moved into PML4 slot 510 (`0xffff_ff00_…`) so the kernel's own
  virtual mappings cannot collide with Limine's huge-page HHDM window (slot 511 is the image).
- **`aios-kernel-run` produces a Limine ISO.** It builds the kernel
  (`cargo build --target x86_64-unknown-none --release`), stages `boot/aios-kernel`,
  `boot/limine.conf`, the Limine BIOS/UEFI CD stages and `EFI/BOOT/BOOTX64.EFI`, creates the
  hybrid ISO with `xorriso` + `limine bios-install`, and boots it in QEMU — **UEFI/OVMF (real
  GOP) by default**, legacy BIOS otherwise. Tool paths/behaviour are overridable with
  `AIOS_LIMINE_DIR`, `AIOS_XORRISO`, `AIOS_QEMU`, `AIOS_QEMU_UEFI`, `AIOS_SKIP_QEMU`.

### Fixed
- **`font8x8` 0.3.1 does not build for `x86_64-unknown-none`** (it is a `std` crate and uses
  `print!`/`println!` in its `Display`/`Debug` impls). Replaced by a vendored public-domain
  8x8 bitmap font in `aios-kernel/src/font8x8.rs` (`BASIC: [[u8; 8]; 128]`), so the kernel
  stays `no_std` and dependency-free for rendering.

### Verified
- `cargo build --target x86_64-unknown-none --release` — clean, 0 warnings; ELF entry
  `0xFFFFFFFF800047B0` (Limine higher half).
- UEFI (OVMF) QEMU run: `framebuffer = 1280x800 pitch=5120 bpp=4 usable=true`,
...
- Legacy-BIOS QEMU run: identical milestone lines.
- Screendump pixel histogram: 60 612 foreground / 963 324 background / 64 self-check pixels —
  the console actually renders on GOP.
...
### Added
- **`Priority::from_name`** (`aios-process-mgr::task`) — case-insensitive priority parsing including `"default" → Normal`, used by the bridge `AdjustPriority` target parser (+2 tests).
- **`BlockRegistry::swap_binary(id, binary, version)`** (`aios-block-mgr::registry`) — atomic binary/version replacement preserving the block id and re-hashing the payload; the primitive behind bridge and GUI hot-swap (+2 tests).
- **Bridge intent implementations** (`aios-bridge::server`): `AdjustPriority` (parses `pid + priority` target strings), `HotSwap` (loads a `<name>_<version>.wasm` from disk, `LiveUpdateEngine::perform_swap` → `registry.swap_binary`), `WorkflowExecution` (sequential recursive intent execution), and a real derived **CPU metric** (`running_count / (cores × 2)`). Built on dedicated `LiveUpdateEngine`/`IpcBus` instances owned by `BridgeContext` (+7 tests).
- **WorkflowExecution in the LLM classifier** (`aios-bridge::intent_engine`) — `parse_llm_response` now decodes `{"intent":"WorkflowExecution","steps":[...]}` and the classifier prompt advertises the variant.
- **WebSocket telemetry auth** — `/ws/telemetry` accepts an optional `?token=<session>` query validated against the bridge `auth` layer when users exist; the route stays in the middleware allowlist, enforcement happens inside the handler.
- **Live GUI kernel runtime** (`aios-gui::runtime::GuiRuntime`) — real `Scheduler` (round-robin, aging 5000 ms, 100 ms slice), `BlockRegistry` seeded with native modules + `boot_discover` from `AIOS_BLOCKS_DIR`, real `Watchdog` (heartbeat generator every 1 s, secret `aios_gui_secret`), shared `IpcBus` fed by real OS-thread kernel processes (`ai_orchestrator`, `io_handler`, `health_monitor`, `telemetry_agg`) via `spawn_real_process`/`TerminateFlag`/`SuspendFlag`, `LiveUpdateEngine`, `HotReloader` and a local `BlockMarketplace` (`official` repo with 4 internal offers). Threads start only from `start_runtime()` (called in `main`), never in tests; test builds use a unique temp blocks dir (+5 tests).
- **GUI actions wired to real data** (`aios-gui::app`) — process list/RAM/IPC traffic/watchdog state refreshed from the runtime every 250 ms; `kill/suspend/resume` drive the scheduler; `load/unload/update/uninstall` operate on the real store + registry; Hot-Swap executes a live-update swap against the installed store copy and renders a swap-history panel in the Blocks tab; install writes real `<name>_<version>.wasm` + sidecar manifests.
- **`aios-studio` WS token** — `app.js` appends `?token=<stored session>` to the telemetry WebSocket URL.
- Crate wiring: `aios-bridge` now depends on `aios-ipc` + `aios-live-update`; `aios-gui` on `aios-store` + `aios-live-update` + `aios-block-mgr`.

### Verification
- `cargo test -p aios-process-mgr` (75), `-p aios-block-mgr`, `-p aios-bridge` (14) and `-p aios-gui` (18) all green; `cargo clippy --workspace` **0 warnings**; `cargo fmt --all` applied.
...
- **Hardware Inspector on tab 1 (System & HW) no longer shifts on each visit.** The hot-plug toast strip reserved `min(toasts,4)+1` rows, growing 1→5 lines per new toast and reflowing the whole device table underneath — every switch, another line and the content jumped. The strip now reserves a **fixed** 5-row block whenever toasts exist (displays the latest 4), so the device table stays put and new toast lines replace old ones in-place instead of shifting the layout.
- **No more repeated phantom re-provisioning on Windows.** On the test machine a `USB 0000:0000 (unknown)` PnP entry (a device without any vendor/product identity) flapped every few seconds, firing `WM_DEVICECHANGE` → full re-detect → fingerprint diff → provisioning attempt → toast, over and over — the "new line appears every time I open tab 1" spam. Three fixes: (1) `HardwareProfile::detect_usb` now drops `0000:0000` entries as enumeration artifacts (new `is_identifiable_usb` helper, unit-tested), so they never reach the fingerprint diff; (2) the native hotplug push is coalesced in `HotplugMonitor` — bursts of pushes trigger at most one scan per `poll_ms` instead of one scan per event; (3) the `HAL: Detected …` / `HAL: NVIDIA GPU detected` INFO lines are emitted once per process instead of every rescan.
- **F1 help overlay finally rendered in the kernel TUI.** `show_help` was toggled by `F1`/`?` but never drawn — pressing F1 did nothing visible despite the F-key "Help" label and the INTERFACE/ARCHITECTURE docs already describing the overlay. `draw()` now renders a full-screen opaque `AIOS Help` block (global + per-tab shortcuts; the previous background is cleared so text never blends). Hold behavior: while F1 is held the overlay stays open; it closes on `F1`/`?`/`Esc`/`h`. Covered by `help_overlay_renders_on_all_sizes` (40×12…120×30).
- **`F10` manual hardware re-probe bound in the kernel TUI.** The System & HW tab's inspector text and INTERFACE docs already promised "press F10 to re-probe", but `F10` had no handler (the F-key bar label said `Quit`, which `q` already does). `F10` now calls `refresh_hw()` (full `HardwareProfile::detect()` + engine rescan) on any tab; the F-key bar label changed `10Quit` → `10Rescan`, and the render test asserts it.
