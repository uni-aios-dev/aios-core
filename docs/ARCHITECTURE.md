...
- **Scheduler** (`scheduler.rs`): `DistributedScheduler` — a background **heartbeat thread** announces `Hello(self_info)` to peers on an interval; peers reply `Metrics`, so every node converges on a live view of the cluster. Liveness: `last_contact` per node; a node silent longer than `failover_threshold` flips to `Offline` in `tick()`. Placement filters online nodes by tier range then applies the strategy (LeastLoaded uses `load_fraction`, tie → lowest node id). `spawn`/`kill`/`set_priority` are blocking calls that drain the inbox until the matching ack or `ack_timeout`. `migrate` relocates a tracked process to another node: it spawns the copy on the destination (explicit node or the placement strategy, never the source) and only then kills the original, so a spawn failure leaves the source untouched; a relocate back onto the source node is rejected and the extra copy is re-killed. On node loss `tick()` respawns that node's tracked processes elsewhere (`failover_respawn`). **Metrics authority**: a known node's load is updated only by the dedicated `Metrics` message — the `Hello` snapshot is used only for first join, so a stale idle snapshot cannot overwrite live load. Bounded event log (`events()`, last 100).
...
- **Tests**: 16 unit (protocol 4, transport 2, executor 1, scheduler 8, config 2) + 9 integration (`tests/scheduling.rs`: two-node spawn/kill, round-robin, least-loaded, TCP loopback, failover respawn, priority control, process migration, migrate error paths, unknown-node/no-peers errors) + 1 doc test.
