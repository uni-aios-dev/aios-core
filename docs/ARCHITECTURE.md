...
**TUI crash-resilience (v2.31.1):** all draw-path and handler `Mutex` locks (`app.state`, `bridge.registry`, `bridge.scheduler`, `ai_*`) go through the poison-tolerant `locked()` helper (`app_state.rs`), so a background bridge worker that panics while holding those mutexes can no longer panic the renderer. `run_tui` additionally wraps the `run()` loop in `std::panic::catch_unwind` and unconditionally restores the terminal (disable raw mode, leave the alternate screen, show cursor), returning `Err("AIOS TUI crashed: …")` on a caught panic instead of leaving the console in raw mode.

### Hot-plug event loop (`hotplug.rs`)

`HotplugMonitor` — a background thread (one per UI surface) that re-detects the attached `HardwareProfile` and diffs the fingerprint set against the previous poll, emitting `HotplugEvent::Added/Removed` over an `mpsc` channel. The first scan only records the baseline, so startup is silent. Detection is adaptive: on Linux a cheap signal (`cheap_signal`/`dir_signal_hash`) hashes the mtimes of `/sys/bus/{usb,pci,nvme}/devices` — directories that only change when a device is attached/removed — so a full `detect()` runs immediately when the device tree moves (cadence `signal_poll_ms`, default 250 ms), with `poll_ms` (default 1000 ms) as the safety-net full-scan interval; platforms without such a signal use the fixed `poll_ms` cadence. Native push events (Windows `WM_DEVICECHANGE`) are coalesced to at most one full scan per `poll_ms`, so a flapping device never rescan-spams; `HardwareProfile::detect_usb` also skips `0000:0000` PnP artifacts so phantom entries never reach the fingerprint diff. The monitor is never `Send`-shared into the engine; each UI tick drains the channel and applies events on the UI thread: `Added` → `provision_blocking`, `Removed` → `AutohalEngine::remove_device` (unloads the WASM instance + tracked device but keeps the cached driver, so a replug re-provisions instantly). The monitor stops cleanly on `Drop`.
...
Kernel v2.29.0 adds milestones 3 and 4 to the bare-metal track: `sched.rs` - PIT-tick round-robin scheduler (switch every TIMER_HZ/4 ticks) doing frame-copy context switches inside the timer ISR, plus a ring-0 worker; `user.rs` - two ring-3 demo programs (raw machine-code loops over `int 0x80`) mapped user-mode at CODE_BASE 0x40000000 / STACK_TOP 0x7F000000; `ipc.rs` - per-pid mailboxes (MAX_PID=4 x MAILBOX_DEPTH=16) behind the DPL-3 `int 0x80` gate (IDT flags 0xEE) with a packet header mirroring `aios_core::ipc_protocol`; runtime proof lines `[stats] switches/sent/recv` every 5 s, asserted by `scripts/qemu-smoke.ps1`. Since v2.38.11 the `SYS_SEND` destination is the target's **scheduler slot** (worker = 1, tasks = 2/3/4 in spawn order) — programs send to peer-slot constants `SLOT_A`/`SLOT_B`, not `PID_*`, and the mailbox counters/`mailbox_len` feed the on-screen dashboard.
## Bare-Metal Kernel (`aios-kernel`, `aios-kernel-run`) — Limine + GOP since v2.34.0

A fresh `x86_64-unknown-none` microkernel. Since **v2.34.0** it boots as a **Limine-protocol** ELF from a hybrid BIOS+UEFI ISO (the same Limine the Live image uses) and renders through a native **GOP/VBE framebuffer** console; the earlier `bootloader::BiosBoot` + VGA-text path was removed. It is the seed of a self-hosted kernel and provides console I/O, interrupts (GDT/TSS, IDT, PIC remap, PIT timer, PS/2 keyboard), paging (own page-table walker + frame allocator + kernel heap), preemption and kernel IPC.

- `aios-kernel`: `no_std`/`no_main` crate. Boot contract:
  ...
  - `crt` — **removed in v2.36.0**: the strong `memcpy`/`memmove`/`memset`/`memcmp`/`bcmp` this module defined produced a circular PLT for `memset` and hung the boot (see BUGS); the kernel now relies on `compiler_builtins`' weak `mem*`.
  - `pci` — PCI configuration-space access over the legacy `0xCF8`/`0xCFC` ports; full bus/device/function enumeration into a fixed array of `PciDevice` records (ids, class/subclass/prog-if, BARs, IRQ) with `class_name()` decoding — the discovery layer for the AHCI/NVMe and xHCI drivers.
  - `ahci` — AHCI (SATA) block driver (v2.36.0): enables PCI memory space + bus-mastering, resets the HBA (`GHC.HR`/`AE`), builds per-port command-list / FIS-receive / command-table frames with `memory::alloc_frame`, starts each implemented port, reads `PxSIG` after start (skipping ATAPI/SEMB/PM), issues `IDENTIFY DEVICE` (word-swapped model + 28/48-bit capacity) and moves sectors with `READ`/`WRITE DMA EXT` over a PRDT entry; `AhciDrive`/`read_sectors()`. The controller BAR is reached through `memory::map_mmio`.
  - `nvme` — NVMe block driver (v2.37.0): enables PCI memory space + bus-mastering, maps the 64-bit BAR0 through `memory::map_mmio`, resets the controller (`CC.EN=0`), programs `AQA`/`ASQ`/`ACQ` and waits for `CSTS.RDY`; issues `Identify Controller` (ASCII `MN`/`SN`), `Create I/O CQ`/`SQ`, `Identify Namespace` (NSZE + LBA format) and polled `Read` commands (PRP1 into one DMA frame); `NvmeDrive`/`read_blocks()`.
  ...
  - `interrupts` — `aios_interrupt_common` C shim (`aios_handle_interrupt`) dispatches by vector: fatal faults (0/6/8/13/14) print and halt, IRQ range `0x20..=0x2F` is EOI'd, IRQ0 (PIT) bumps `TICKS` at 100 Hz and sets `IRQ32_SEEN`, IRQ1 (keyboard) stores the last scancode. `init_pic` remaps the 8259 PIC to `0x20`/`0x28` and masks all but IRQ0/IRQ1; `init_pit` programs channel 0 for 100 Hz. `pit_count`/`delay_ms`/`wait_for_irq32` drive the same countdown directly (no IRQ involved) for the software-tick fallback.
  - `sched` — PIT-tick round-robin over `MAX_TASKS = 5` traps frames (slot 0 = idle). Ring-3 tasks are preemptive (switched at any timer IRQ); ring-0 tasks switch only cooperatively via `yield_kernel` (`int 0xfa`). **Cooperative-only on PIC-less boards:** if the legacy PIC never delivers IRQ0 (UEFI IO-APIC routing), there are no preemption interrupts, so a ring-3 task that never syscalls yields the tick economy to nobody — demo programs therefore keep sleeping (`SYS_SLEEP`) forever and must never end in a busy loop. `idle_loop` detects `IRQ32_SEEN == false` and synthesizes the 100 Hz tick cadence itself by polling the PIT countdown, so sleeps/`[tick]`/heartbeat still advance.
  - `memory` — paging core over the Limine HHDM (`physical_offset = hhdm.offset`). `translate` walks the active page tables (PML4/PDPT/PD/PT, huge pages) reading physical memory through the HHDM; `map_page`/`unmap_page` edit tables and allocate page-table frames on demand, flushing the TLB with `invlpg`; `init` builds a bump frame allocator over the Limine `MEMMAP_USABLE` regions supplied as `MemRegion { start, end }`; `selftest` maps a scratch page, writes/reads a pattern and verifies `translate` before and after unmap. `map_mmio(phys, size)` maps device BAR pages into a dedicated window at `0xffffff00_2000_0000` (PML4 slot 510) — the HHDM covers RAM only; `physical_to_virtual(phys)` returns the HHDM alias used for DMA buffers.
  - `heap` — 2 MiB kernel heap (`HEAP_START = 0xffff_ff00_0000_0000`) mapped page-by-page by `memory::map_page`; `#[global_allocator]` free-list allocator (first-fit with split, coalescing-with-next, spin-lock guarded); `init_heap` sets up the single free block, `test_heap` allocates `Vec`/`String`/`Box` and stress-cycles 200 strings.
  - `tui` — lock-free on-screen dashboard (v2.38.11): `render()` paints a `TUI_ROWS = 8` panel above the scrolling console once per second from `idle_loop`. Rows: banner/clock (kernel identity + tick counter), scheduler (HW-IRQ `100 Hz` vs SOFT-tick mode, `switch_count`, current pid, per-task state from `sched::task_present`/`task_asleep`), IPC (`syscalls::stats` sent/recv + per-task `mailbox_len`), drivers (status of `G_AHCI`/`G_NVME`/`G_XHCI`: `-1` absent, `0` failed, `1` ok, `2` read-verified), memory (`memory::frames_allocated`) and a 1 Hz tick progress bar. All reads are of atomics or integral counters — no locks, safe to call on PIC-less boards.
  - `main` — reads the Limine framebuffer/rsdp/hhdm responses, runs the framebuffer read-back self-check, filters the memory map into `[MemRegion; 64]`, then initializes memory + heap, GDT/TSS, IDT, PIC and PIT, `sti`, prints an on-screen `[probe] irq32_seen/ticks delta` liveness readout, enumerates the PCI bus and brings up the AHCI and NVMe storage drivers (after `sti`, so faults are handled), schedules the kernel worker and three ring-3 tasks, declares the atomic driver-status statics `G_AHCI`/`G_NVME`/`G_XHCI` (stamped by each driver's init), and enters `idle_loop` where the heartbeat square and `tui::render()` are driven.
...
- Flash to a real USB stick: run `scripts\flash-usb.ps1` as Administrator (it writes the image to `\\.\PhysicalDriveN`, verifies SHA-256 and the MBR `55 AA` signature). A bare-metal AIOS stick boots on both legacy BIOS and UEFI laptops.
- Verify: `cargo run --release` in `aios-kernel-run` — serial shows `aios-kernel: Limine handoff`, HHDM offset, RSDP, `framebuffer = WxH pitch=… bpp=… usable=true`, `framebuffer self-check OK (readback 0x…)`, `usable memory regions = N`, `paging selftest OK.`, `heap: …`, `interrupts online.`, `[sched] ready`, `three ring-3 tasks armed`. A QEMU `screendump` confirms the console draws (foreground/background pixel histogram).
