...
- `aios-kernel`: `no_std`/`no_main` crate. Entry point via `bootloader_api::entry_point!(kernel_main, config = &BOOTLOADER_CONFIG)`; `BOOTLOADER_CONFIG` enables `mappings.physical_memory = Some(Mapping::Dynamic)` so low physical memory (e.g. the VGA text buffer at `0xB8000`) is reachable at `physical_memory_offset`.
  ...
  - `memory` — paging core. `translate` walks the active page tables (PML4/PDPT/PD/PT, huge pages supported) reading physical memory through the bootloader's physical-memory map; `map_page`/`unmap_page` edit tables and allocate page-table frames on demand, flushing the TLB with `invlpg`; `init` builds a bump frame allocator over the boot info `Usable` memory regions; `selftest` maps a scratch page, writes/reads a pattern and verifies `translate` before and after unmap.
  - `heap` — 2 MiB kernel heap (`HEAP_START = 0xFFFF840000000000`) mapped page-by-page by `memory::map_page`; `#[global_allocator]` free-list allocator (first-fit with split, coalescing-with-next, spin-lock guarded); `init_heap` sets up the single free block, `test_heap` allocates `Vec`/`String`/`Box` and stress-cycles 200 strings.
  - `main` — boots, initializes serial/VGA, prints memory regions, physical-memory offset, framebuffer and RSDP, then initializes the memory manager and heap (milestone 2), enables GDT/TSS, IDT, PIC and PIT, `sti`, and enters `idle_loop` printing `tick Ns` every second and decoded keys.
