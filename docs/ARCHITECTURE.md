...
- `aios-kernel`: `no_std`/`no_main` crate. Boot contract:
  ...
  - `sched` — PIT-tick round-robin over `MAX_TASKS = 5` traps frames (slot 0 = idle). Ring-3 tasks are preemptive (switched at any timer IRQ); ring-0 tasks switch only cooperatively via `yield_kernel` (`int 0xfa`). If the legacy PIC never delivers IRQ0 on a UEFI board, `idle_loop` detects `IRQ32_SEEN == false` and synthesizes the 100 Hz tick cadence itself by polling the PIT countdown, so sleeps/`[tick]`/heartbeat still advance.
