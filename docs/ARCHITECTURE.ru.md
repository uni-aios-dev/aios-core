...
- `aios-kernel`: крейт `no_std`/`no_main`. Точка входа через `bootloader_api::entry_point!(kernel_main, config = &BOOTLOADER_CONFIG)`; `BOOTLOADER_CONFIG` включает `mappings.physical_memory = Some(Mapping::Dynamic)`, поэтому нижняя физическая память (например, текстовый буфер VGA по `0xB8000`) доступна по `physical_memory_offset`.
  ...
  - `idt` — IDT на 256 записей, строится из ассемблерной `aios_handler_table` (стабы векторов в `.text`, таблица в `.data.rel.ro` с релокациями, применяемыми загрузчиком при загрузке); для гейта 8 используется double-fault IST.
  ...
  - `memory` — ядро paging. `translate` обходит активные таблицы страниц (PML4/PDPT/PD/PT, большие страницы поддерживаются), читая физическую память через карту загрузчика; `map_page`/`unmap_page` редактируют таблицы и выделяют кадры для таблиц по требованию, сбрасывая TLB через `invlpg`; `init` строит bump-аллокатор кадров по `Usable`-регионам из boot info; `selftest` мапит тестовую страницу, пишет/читает паттерн и проверяет `translate` до и после unmap.
  - `heap` — куча ядра 2 МиБ (`HEAP_START = 0xFFFF840000000000`), замаплена страница за страницей через `memory::map_page`; `#[global_allocator]` free-list аллокатор (first-fit со сплитом, коалесценцией со следующим блоком и spin-lock); `init_heap` создаёт единый свободный блок, `test_heap` выделяет `Vec`/`String`/`Box` и стресс-цикл из 200 строк.
  - `main` — загрузка, инициализация serial/VGA, вывод memory regions, смещения физической памяти, framebuffer и RSDP, затем инициализация memory manager и кучи (веха 2), включение GDT/TSS, IDT, PIC и PIT, `sti` и вход в `idle_loop`, который печатает `tick Ns` каждую секунду и раскодированные клавиши.
