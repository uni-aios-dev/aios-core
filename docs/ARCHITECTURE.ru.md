...
- `aios-kernel`: крейт `no_std`/`no_main`. Контракт загрузки:
  ...
  - `main` — читает ответы framebuffer/rsdp/hhdm Limine, выполняет self-check чтения framebuffer, фильтрует карту памяти в `[MemRegion; 64]`, затем инициализирует память и кучу, GDT/TSS, IDT, PIC и PIT, `sti`, планирует kernel worker и три ring-3 задачи и входит в `idle_loop`.
