...
**Вкладка 3 — AI Console**: чат с LLM, `i` — режим запроса, `Enter` — отправка, `Esc` — выход, `Up`/`Down` — история промптов (последние 50), `h` — панель справки; слэш-команды `/help /status /clear /history /system /model /backend /key /temp /tokens`; внизу — бэкенд/модель/температура/токены/состояние; вывод переносится по ширине с циановыми промптами и красными ошибками; изменения бэкенда/модели/ключа асинхронно пересоздают общий `LlmEngine`
...
- **AI Studio (F3)**: асинхронный чат с LLM — список сообщений, отправка по Enter (фокус сохраняется), слэш-команды `/help /backend /model /key /temp /tokens /clear /history`, строка статуса (бэкенд/модель/температура/токены/busy); запросы выполняются в фоновой tokio-задаче, интерфейс остаётся отзывчивым
