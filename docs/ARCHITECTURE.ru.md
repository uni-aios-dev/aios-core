...
- `aios-kernel`: крейт `no_std`/`no_main`. Контракт загрузки:
  ...
  - `sched` — round-robin по тикам PIT над `MAX_TASKS = 5` кадрами (слот 0 = idle). Ring-3 задачи вытесняемы (переключаются по любому таймерному IRQ); ring-0 задачи переключаются только кооперативно через `yield_kernel` (`int 0xfa`). Если legacy PIC на UEFI-плате не доставляет IRQ0, `idle_loop` обнаруживает `IRQ32_SEEN == false` и сам синтезирует тактовость 100 Гц опросом счётчика PIT, поэтому sleep/`[tick]`/heartbeat продолжают идти.
