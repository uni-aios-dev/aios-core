...
- Область отображения текста страницы (рендеринг по WHATWG-стандарту через html5ever: заголовки `#`, списки `•`/`1.`, `pre` без потерь, таблицы `|`, `hr`, изображения как `[alt]`)
- Прокручиваемый список ссылок с выбором (индикатор `>>`); ссылки резолвятся относительно базового URL страницы, дедуплицируются, не-web-схемы (`javascript:`, `mailto:`, `#якорь`) отфильтрованы
- Фоновая загрузка через reqwest blocking + HtmlParser из aios-browser; результаты поиска DuckDuckGo отображаются как страница
