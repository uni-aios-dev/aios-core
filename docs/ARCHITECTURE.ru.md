...
- `aios-kernel`: крейт `no_std`/`no_main`. Контракт загрузки:
  ...
  - `main` — читает ответы framebuffer/rsdp/hhdm Limine, выполняет self-check чтения framebuffer, фильтрует карту памяти в `[MemRegion; 64]`, затем инициализирует память и кучу, GDT/TSS, IDT, PIC и PIT, `sti`, перечисляет шину PCI и поднимает драйверы хранилища AHCI и NVMe (после `sti`, чтобы исключения обрабатывались), планирует kernel worker и три ring-3 задачи и входит в `idle_loop`.
  - Panic-обработчик печатает в framebuffer-консоль и serial, затем уходит в `halt_loop`.
- `aios-kernel-run`: собирает ядро (`cargo build --target x86_64-unknown-none --release`), раскладывает дерево Limine ISO (`boot/aios-kernel`, `boot/limine.conf`, CD-стадии Limine BIOS/UEFI, `EFI/BOOT/BOOTX64.EFI`), создаёт гибридный ISO через `xorriso` + `limine bios-install` и грузит его в QEMU — по умолчанию UEFI/OVMF (настоящий GOP), иначе legacy BIOS. Переопределения: `AIOS_LIMINE_DIR`, `AIOS_LIMINE_TOOL`, `AIOS_XORRISO`, `AIOS_QEMU`, `AIOS_QEMU_UEFI`, `AIOS_SKIP_QEMU`.
