...
- Омнибокс (принимает полный URL, голый хост или обычный запрос — запросы ищутся через DuckDuckGo) с индикатором фокуса
- Область отображения текста страницы (рендеринг по WHATWG-стандарту через html5ever: заголовки `#` — жирным циановым, списки `•`/`1.`, `pre` без потерь, таблицы `|`, `hr`, изображения как `[alt]`)
- Прокручиваемый список ссылок с выбором (индикатор `>>`); ссылки резолвятся относительно базового URL страницы, дедуплицируются, не-web-схемы (`javascript:`, `mailto:`, `#якорь`) отфильтрованы; окно прокручивается за выбором (6 видимых строк) и показывает видимый диапазон в заголовке
- Загрузки выполняются **в фоне** (никогда не блокируют TUI): `load_url`/`navigate_web` запускают поток, который кладёт результат в исходящий канал `page_cache`, подхватываемый `check_page_cache()` каждый кадр; счётчик поколений загрузки отбрасывает устаревшие результаты
- Ограниченный **кэш страниц** (`WebState.cache`, 20 страниц по ключу URL, старейшие вытесняются) делает повторные визиты и навигацию назад (`b`) мгновенными
- Текст страницы **переносится по ширине терминала** (`wrap_text()`): каждая строка разбивается по границам слов (длинные слова разрываются принудительно), пустые строки и отступы сохраняются; единицы прокрутки равны визуальным строкам; `WebState.wrap_width` задаётся при старте и обновляется при изменении размера терминала
- Фиксированный **навигационный сайдбар** (`SIDEBAR_WIDTH = 26`) слева от панели страницы показывает текущую страницу (отмечается `▸`) и затем историю посещений от новых к старым (`web_nav_entries()`), с компактными обрезанными ярлыками URL (`compact_url_label()`); фокус переключается `\`, `j`/`k` двигают выбор, `Enter`/`o` открывают запись, `Esc` возвращает к ссылкам; ширина переноса считается через `web_page_width()` = ширина терминала минус сайдбар, рамки панели и префикс в 2 колонки
- **Полноценный браузер по запросу**: `B` открывает текущую страницу (а `n` — выбранную ссылку) в настоящем окне `aios-webview` (WebView2 — JS/CSS/картинки). Handle живёт в модульном `OnceLock<Mutex<Option<WebBrowser>>>` в `aios-tui`, поэтому окно переиспользуется, пересоздаётся при закрытии, а открытие идёт в фоновом потоке (TUI не блокируется). Текстовый вид не меняется — нативный браузер это запасной выход для полного рендеринга
- Текстовые загрузки используют `http_client()` — десктопный User-Agent Chrome, `Accept: text/html` и таймаут 15с — чтобы избегать бот-блокировок и зависаний
...
- Клавиши: `g`=фокус омнибокса, `Enter`=поиск/навигация (автоснятие фокуса), `o`/`Enter`=открыть выбранную ссылку, `j/k`=навигация, `b`=назад в истории, `u`/`d`=прокрутка ±1 строка, `PageUp`/`PageDown`=прокрутка ±20 строк, `Esc`=снять фокус
...
**Зона подвала**: подсказки горячих клавиш (q=Выход, 1-7=Вкладка, Alt+1-7=Вкладка в любом режиме ввода, j/k=Навигация, K=Убийство, U=Выгрузка, L=Загрузка, H=Горячая замена, F1=Справка, :=Команда, s=Телеметрия, x=Статус, r=Обновление, W=GUI)
...
Горячие клавиши: `q`=Выход, `1-7`=Вкладка, `Alt+1-7`=Вкладка даже при вводе в Shell/URL, `j/k`=Навигация, `K`=Убийство процесса, `r`=Обновить, `s`=Запись телеметрии, `x`=Статус системы, `W`=Запуск дашборда GUI, `F1`/`?`=Справка, `:`=Команда Shell, ↑/↓=История Shell, Вкладка Web: `g`=фокус омнибокса, `o`/`Enter`=Открыть ссылку, `b`=назад в истории, `u`/`d`=прокрутка ±1 строка, `PageUp`/`PageDown`=прокрутка ±20 строк, `Esc`=снять фокус
...
Нативный дашборд на egui/eframe с 7 вкладками: Overview, Processes, Blocks, Marketplace, Metrics, Deps, **Browser**. Вкладка **Browser** (F7) содержит омнибокс, кнопки Back/Forward и переключатель Open/Close, управляющие нативным окном `aios-webview`; первая навигация автоматически открывает браузер. Горячая клавиша `W` в обоих TUI запускает дашборд GUI через `aios_webview::launcher::launch_gui()`.
...
| n | Изменить сетевые настройки (режим ввода `key=value`, команда `net_set` по IPC) |
| r | Переопределить оборудование |
| Space | Пауза/возобновление прокрутки логов |

### AI Console (вкладка 3, Фаза 43 / v2.6.0)
...
- Система слэш-команд в `TuiApp::handle_ai_command`: `/help /status /clear /history /system /model /backend /key /temp /tokens`; смена бэкенда/модели/ключа асинхронно пересоздаёт движок через `apply_config_async`
...
- Строка состояния `backend | model | temp | tokens | state` (`thinking...` / `done: Nms` / ошибка); `/status` выводит конфигурацию и найденные локальные GGUF-модели через `aios_llm::local::detect_local_models`
