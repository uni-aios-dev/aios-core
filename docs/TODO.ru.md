...
- [ ] **Веха 6** (следующая): нативный ввод + хранилище — перечисление PCI, USB-HID (xHCI) клавиатура/мышь вместо PS/2, драйвер блоков AHCI/NVMe и модуль `crt` для явных `memcpy`/`memset`/`memmove`/`memcmp`.