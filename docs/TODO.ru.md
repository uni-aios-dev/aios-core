...
- [ ] Follow-up по надёжности установщика: переключатель `verbose`, журнал установки на целевом диске, защита от повторной установки и фиксация размера root — **сделано (v2.33.1)** (`aios.verbose`, `/aios-install.log` с `AIOS INSTALL COMPLETE`, `Type YES to FORCE reinstall`, размер root в журнале; безлюдный QEMU-приём повторно проверен). Осталось: цели LUKS/LVM/software-RAID, retry/resume после прерванной копии (сейчас — повторная копия) и безлюдный приёмочный прогон `--auto` на реальном (legacy BIOS) железе.
...
## Дорожная карта микроядра (`aios-kernel`)

- [x] **Веха 0** (v2.26.0): голое ядро `x86_64-unknown-none` загружается под QEMU с последовательной (COM1) и VGA консолью; конфиг загрузчика включает маппинг физической памяти, чтобы текстовый буфер VGA был доступен.
- [x] **Веха 1** (v2.27.0): прерывания — GDT/TSS (с double-fault IST), IDT на 256 записей с настоящими обработчиками (page fault, double fault, GPF), ремап 8259 PIC, таймер PIT (100 Гц → `TICKS`), клавиатура PS/2; idle-цикл `sti`/`hlt` печатает `tick Ns` и раскодированные клавиши.
- [x] **Веха 2** (v2.28.0): страничная память — собственный обход таблиц страниц (`memory::translate`) по карте физической памяти загрузчика, `memory::map_page`/`unmap_page` с выделением кадров под таблицы по требованию, bump-аллокатор кадров по `Usable`-регионам, куча ядра 2 МиБ (`heap`, `#[global_allocator]` free-list аллокатор, `Box`/`Vec`/`String`).
- [x] **Веха 3** (v2.29.0): вытеснение — round-robin-планировщик по тикам PIT (`sched.rs`, переключение каждые TIMER_HZ/4 тиков) через frame-copy контекста внутри ISR; ring-0 worker-поток ядра + две ring-3 пользовательские программы с user-мапингом (`user.rs`, CODE_BASE 0x40000000, STACK_TOP 0x7F000000); селекторы `gdt::USER_CS/USER_DS`.
- [x] **Веха 4** (v2.29.0): IPC ядра — почтовые ящики на pid (`ipc.rs`, MAX_PID=4 × MAILBOX_DEPTH=16) за вентилем `int 0x80` c DPL-3 (флаги IDT 0xEE); syscall'ы SYS_SEND/SYS_RECV, заголовок пакета зеркалит `aios_core::ipc_protocol`; периодические proof-строки `[stats] switches/sent/recv` и headless-проверка `scripts/qemu-smoke.ps1`.
- [x] **Веха 5** (v2.34.0): загрузка Limine + GOP — гибридный BIOS+UEFI ISO, ELF по протоколу Limine (`_start`, секции `.requests`, `linker.ld`), HHDM-маппинг + карта памяти из Limine, нативная framebuffer-текстовая консоль, сборщик ISO `aios-kernel-run` + запускатор QEMU (OVMF/legacy); проверено под UEFI GOP и legacy BIOS с гистограммой пикселей screendump.
...
- [x] **v2.38.1**: исправлен `yield_kernel()` — `asm!("int $$0xfa")` →
  ...
  где `$` не является валидным префиксом немедленного значения.
  Это создает невалидную инструкцию (#UD) на реальном железе,
  блокируя кооперативное планирование через `yield_kernel()`.
- [x] **v2.38.1**: добавлен `sti; hlt` во все циклы ожидания
  в `main.rs`. Голый `hlt` без явного `sti` мог оставить прерывания
  отключенными на реальном оборудовании, где состояние CPU
  отличается от QEMU.
- [x] **v2.38.11**: экранный дашборд микроядра (`src/tui.rs`,
  `TUI_ROWS = 8`) — живые строки часы/планировщик/IPC/драйверы/память +
  прогресс-бар тика, без блокировок, рисуется из `idle_loop` на 1 Гц;
  статики статуса драйверов `G_AHCI`/`G_NVME`/`G_XHCI`. Заодно исправлена
  маршрутизация демо-IPC (порядок спавна теперь A → B → C, явные адресаты
  `SLOT_A`/`SLOT_B`) — proof-строки `[ipc] send/recv` наконец появляются.