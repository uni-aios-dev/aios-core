...
## v2.38.11 — Экранный дашборд микроядра + настоящий IPC ping-pong (2026-09-26)

Микроядро теперь рисует живой статус-дашборд над скроллящейся консолью
(`src/tui.rs`, `TUI_ROWS = 8`): строка баннера/часов, строка планировщика
(режим тика, переключения, текущий pid, состояние задач), строка IPC
(итоги send/recv + заполненность почтовых ящиков задач), строка драйверов
(статусы AHCI/NVMe/xHCI из `G_AHCI`/`G_NVME`/`G_XHCI`), строка памяти
(выделенные кадры) и прогресс-бар тика 1 Гц. Отрисовка без блокировок,
вызывается из `idle_loop` раз в секунду — работает и на PIC-less ноутбуке
MSI, где есть только software-тики.

### Добавлено
- `aios-kernel/src/tui.rs` — `render()` рисует панель. Источники данных —
  статики/счётчики в `interrupts`, `sched`, `syscalls`, `memory`, `xhci` и
  новые атомарные регистры статуса `G_AHCI`/`G_NVME`/`G_XHCI`, проставляемые
  кодом инициализации каждого драйвера.
- `main.rs` — объявляет статики статуса драйверов (-1 нет / 0 сбой / 1 ок /
  2 чтение-проверено), выставляет их в init-блоках AHCI/NVMe/xHCI и зовёт
  `tui::render()` раз в секунду внутри `idle_loop`.
- `sched::task_present`/`task_asleep`, `syscalls::mailbox_len`,
  `memory::frames_allocated`, pub `console::{SCALE, GLYPH_W, GLYPH_H}` +
  `text_height()`.
- `console.rs` уменьшает свой бюджет строк на `TUI_ROWS + 1`, чтобы панель
  дашборда и нижний heartbeat-overlay не пересекались со скроллом текста.

### Исправлено
- **Демо-IPC молча маршрутизировалось не туда.** `user::init()`
  спавнил программы в порядке A, C, B, поэтому runtime-слоты больше не
  совпадали с литералами `PID_*` в демо-машинах: `SYS_SEND` задачи A
  попадал в её же слот (`src == dst`, отброшен), а у B — в ящик kernel
  worker'а; ping-pong никогда не печатал строку `[ipc]`. Теперь программы
  спавнятся A → B → C (worker владеет слотом 1, задачи — слоты 2/3/4), и
  A/B шлют на `SLOT_B`/`SLOT_A` явно.

### Проверено
- `cargo build` (ядро, release): OK; `cargo clippy`: 0 предупреждений;
  `cargo fmt` применён. QEMU (OVMF/GOP): A/B/C ложатся на слоты 2/3/4,
  затем появляются `[ipc] send 3->2`, `send 2->3`, `recv 2<-3`, `recv 3<-2`
  при `val=8`; дашборд рисуется и обновляется с каденсом тиков; паники/остановов нет.

## v2.38.10 — Зависание демо ring-3 и следы heartbeat исправлены (2026-09-25)

Два дефекта реального железа из загрузки v2.38.9 на MSI:

### Исправлено
- **Демо pid3 парковалось busy-spin'ом и замораживало ОС на платах без
  PIC.** `build_pid3` заканчивался `jmp $` (бесконечный спин). Без 8259 PIC
  нет вытесняющего планирования (`idle_loop` — единственный источник
  software-tick и heartbeat), поэтому задача, не делающая yield, останавливает
  `TICKS`, морит голодом спящие задачи и замораживает всё ядро. pid3 теперь
  зациклен на свой `SYS_SLEEP`-цикл (`jmp_self` удалён) — планировщик
  остаётся кооперативным навсегда. В QEMU аппаратный PIT скрывал это,
  поэтому баг раньше не проявлялся.
- **Скролл консоли тащил пиксели overlay вверх.** `scroll_up` на каждом
  переходе строки memmove'ил весь кадр, поэтому копии зелёного квадрата
  heartbeat (и PIT-бара сверху) сдвигались вверх — «зелёные квадраты едут
  вверх и копятся». Консоль теперь резервирует нижнюю строку `GLYPH_H` как
  фиксированную полосу overlay, а `scroll_up(pixels, fill, region_height)`
  ограничен областью консоли — overlay не трогается.

### Проверено
- `cargo build` (ядро, release): OK; `cargo clippy`: 0 предупреждений;
  `cargo fmt` применён.

## v2.38.9 — Видимый сэмплинг ring-3 IPC ping-pong (2026-09-25)

Демоверсия IPC уже работает на железе: задача 1 (`SYS_SEND` → задачи 2, затем
`SYS_RECV`) и задача 2 (`SYS_RECV`, `SYS_SEND` → задачи 1) обмениваются
монотонно растущим словом-нагрузкой каждый цикл через почтовые ящики на pid.
На консоли это не было видно, так как строки-доказательства логировались
только при значении, кратном `SAMPLE_EVERY = 256`, до которого маленькие
значения счётчика демо никогда не дорастали.

### Изменено
- Переименовал `SAMPLE_EVERY: u64 = 256` → `IPC_LOG_EVERY: u64 = 8` в
  `aios-kernel/src/syscalls.rs`. Строки `[ipc] send/recv` + serial теперь
  выводятся каждое 8-е переданное значение (~2 строки/с при темпе демо), что
  делает ping-pong видимым на MSI сквозь все слои, не заваливая консоль.

### Проверено
- `cargo build` (ядро, release): OK; `cargo clippy`: 0 предупреждений.

## v2.38.8 — Тест полной закраски экрана; аудит графики GOP (2026-09-25)

Провёл аудит пути отрисовки ядра по чек-листу «пустой TUI-экран» и добавил
одноразовую полную заливку панели как доказательство:

### Результаты аудита (правки не нужны)
- **Маппинг framebuffer** — ядро рисует через старший полубайс (HHDM)-адрес
  framebuffer, который даёт Limine в `framebuffer.rs`; поверхность уже
  промаплена writable загрузчиком, поэтому конвертация `phys_to_virt` не
  нужна (`gop.rs` в этом лейауте нет). Подтверждено на железе MSI: текст,
  глифы, зелёный квадрат самопроверки и оранжевая полоса шагов рисуются.
- **Вшитый шрифт** — глифы из зашитого битмапа `font8x8::BASIC`
  (public-domain 8×8) в `font8x8.rs`, рисует `draw_glyph` (`console.rs`) без
  внешних библиотек. Эквивалент встроенного PSF.
- **`init_tui_test` / плоская закраска** — ядро никогда не использовало
  `ratatui`; `draw_glyph` + `fill_rect` — самодостаточные `no_std`
  примитивы.

### Добавлено
- **Тест прямой закраски (одноразово при загрузке)** — сразу после
  self-check framebuffer вся панель заливается сплошным тёмно-синим
  (`0x000020C0`) прямо в физический пиксельный массив (без консоли и
  библиотек), затем поверх течёт текст консоли. Доказывает, что вся
  поверхность GOP — включая края/letterbox — напрямую записываема из сырых
  пикселей, чего достаточно для любой отрисовки UI внутри ядра.

### Проверено
- `cargo build --manifest-path aios-kernel/Cargo.toml --target
  x86_64-unknown-none --release`: OK.
- `cargo clippy` (ядро, release): 0 предупреждений.

## v2.38.7 — Ограничение спама `[sched] idle` в консоли (2026-09-25)

С активным software-tick фолбэком на MSI idle-задача синтезирует тик
каждые ~10 мс, и пока все три ring-3 задачи спят, `schedule()` выбирал
слот 0 и печатал `[sched] idle` при каждом вызове — сплошной поток
~100 строк/с, заливавший экран. Строка теперь ограничена максимум одним
выводом в секунду (по `TIMER_HZ` тиков), лог загрузки остаётся читаемым,
а система продолжает циклировать: idle → `pid N woke` → задача работает →
`pid N sleep 20 ticks` → idle.

### Проверено на MSI (v2.38.6)
- Многозадачность ожила на реальном железе: `[ktask] alive #N`
  увеличивается, все три ring-3 задачи просыпаются, выполняют код (`u1` /
  `*` / `u2`) и снова засыпают через `SYS_SLEEP`, а software-tick фолбэк
  (мёртвый legacy PIC / нет IRQ32) держит частоту 100 Гц по счётчику PIT.
...
## v2.38.6 — Software-tick фолбэк для плат с мёртвым 8259 PIC (2026-09-25)

На ноутбуке MSI оба индикатора живости v2.38.5 (PIT-бар справа сверху,
heartbeat 2 Гц) остались статичными: **IRQ0 от PIT вообще не доходит до
CPU**. Legacy 8259 PIC мёртв на этой UEFI-плате (IRQ0 идёт через
НЕ-запрограммированный IO-APIC), поэтому `TICKS` так и остался 0: спящие
задачи не просыпаются, tick-зависимый heartbeat не переключается, а PIT-бар
имеет высоту 0 (невидим). Сам планировщик был здоров — система рухнула
только из-за того, что таймерное прерывание не приходит вообще.

### Добавлено
- **`interrupts::IRQ32_SEEN`** — атомарный флаг, который выставляет
  обработчик IRQ32 при первом реальном срабатывании.
- **`interrupts::pit_count()`** — чтение обратного отсчёта PIT channel-0
  через latch (работает и без IRQ: PIT тактируется напрямую).
- **`interrupts::delay_ms()` / `wait_for_irq32()`** — устойчивые к 16-bit
  wrap busy-ожидания по счётчику PIT.
- **Зонд IRQ на экран** — сразу после `sti` `kernel_main` печатает
  `[probe] irq32_seen=..->.. ticks=..->.. (delta N)` после ~120 мс отсчёта
  по PIT, показывая на экране MSI, приходит ли таймерное IRQ.
- **Software-tick фолбэк в `idle_loop`** — когда `IRQ32_SEEN` ложно (IRQ0
  потерян), idle-задача не делает жёсткий `hlt`, а опрашивает счётчик PIT
  один тиковый период (~10 мс) и, если реальное IRQ не пришло, сама
  увеличивает `TICKS`. Это оживляет sleep-дедлайны, вывод `[tick]` и
  heartbeat 2 Гц на любой плате. Там, где IRQ работает, `IRQ32_SEEN` быстро
  становится true и используется обычный `hlt` — без двойного счёта и
  накладных расходов.
...
## v2.38.5 — Индикаторы живости вне консоли на железе MSI (2026-09-25)

На ноутбуке MSI загрузка полностью проходит (14/14 шагов, планировщик
онлайн, три ring-3 задачи запускаются и засыпают на syscalls), но вывод
текста обрывается на `[sched] idle`. Печать тиков (`[irq32]`, `[tick]`,
`[sched] pid N woke`) идёт через лок консоли/COM, поэтому застывший экран
не отличает «CPU завис» от «заклинило лок консоли». Добавлены два
индикатора, полностью минующих консоль:

### Добавлено
- **PIT tick bar** — обработчик IRQ32 теперь каждый аппаратный тик рисует
  циклический бар 8x64 в правом верхнем углу прямо во framebuffer GOP
  (заливка BG + бар FG высотой `TICKS % 64`), независимо от
  `CONSOLE_LOCK`. Перерисовывается каждый тик, поэтому прокрутка текста
  его не скрывает.
- **Медленный heartbeat** — мигание нижнего правого квадрата-пробы теперь
  2 Гц (переключение по границе полусекунды тиков), а не на каждой
  итерации idle. Потоковая частота ~100 Гц сливалась в стабильную заливку,
  которая выглядела как «не мигает».

### Диагностика
- Если бар сверху справа циклирует (растёт/убывает), а текст стоит на
  месте — CPU жив, заклинило лок консоли.
- Если нижний правый квадрат заметно мигает 2 Гц — idle работает, тики идут.
- Если оба статично зелёные — машина реально зависла (тики не идут).
...
## v2.38.4 — Ядро снова компилируется: починка build.rs/асма + удаление мёртвого и дублированного кода (2026-09-25)

Bare-metal ядро НЕ компилировалось с момента коммитов ring-3 restore
(`80a94dd`/`7d519eb`), поэтому каждая перезапись USB незаметно
записывала устаревший образ от 2026-09-24 — фиксы PIT/try_lock/heartbeat
так и не попадали на флешку, и загрузка не менялась при любых правках кода.

### Исправлено
- **`aios-kernel/build.rs`** — у второго блока асма `r#"..."#` (restore
  ring-0 + ring-3) пропал закрывающий `"#,` и завершающий `);` вызова
  `asm.push_str(...)`, а `aios_handler_table` был объявлен повторно внутри
  строки (дубликат символа). Строка теперь корректно закрыта, хвост
  `.section .data.rel.ro / aios_handler_table:` удалён.
- **`aios-kernel/src/sched.rs`** — `*frame = incoming;` и лишняя `}`
  находились ВНЕ `schedule()` (недостижимо + несбалансированные скобки);
  удалено.
- **`aios-kernel/src/interrupts.rs`** — `fatal`/`halt`/`read_cr2` были
  определены дважды (E0428) из-за плохого слияния; оставлен 2-аргументный
  `fatal` с кадром, удалены 1-аргументный вариант и обе пары дубликатов.
- **`aios-kernel/src/main.rs`** — удалён лишний `use crate::interrupts;`
  (E0255: имя определено дважды вместе с `mod interrupts;`).
- **`aios-kernel` (rustfmt)** — впервые применён `cargo fmt` к этому
  (ранее никогда не проверявшемуся по форматированию) крейту, включая
  форматирование `xhci.rs`.

### Проверено
- `cargo build --manifest-path aios-kernel/Cargo.toml --target
  x86_64-unknown-none --release`: OK.
- `cargo clippy` (ядро, release): 0 предупреждений.
- Заново собраны загрузочный ISO и USB-образ (`aios-kernel-run -- AIOS_SKIP_QEMU=1`).

## v2.38.3 — Постоянный индикатор загрузки (шаги 1-14) + мигающий heartbeat (2026-09-25)

Загрузка на реальном железе теперь показывает точное место остановки без F8 и без serial:
`print_step()` всегда рисует оранжевый квадрат 8x8 на нижней-левой полосе шагов и всегда печатает строку шага,
а heartbeat в idle-цикле мигает нижним-правым пробным квадратом (OK-зелёный / BG-чёрный),
чтобы живую работу CPU было видно даже при затруднении lock-based-консоли.

### Изменено
- **`aios-kernel/src/main.rs`** — `print_step()` больше не зависит от
  `DEBUG_MODE`; всегда ставит оранжевый квадрат прогресса на нижней-левой
  полосе и печатает `STEP n/14`. Добавлен `STEP 14/14: ring-3 idle handoff`
  прямо перед `boot_finished()`/`yield_kernel()`. Heartbeat в idle-цикле теперь
  переключает квадрат (width-8, height-8) между `colors::OK` и `colors::BG`
  каждую итерацию вместо смены цвета в (0,0) (что было невидимо — текст затирал row 0).
- **`aios-kernel/src/framebuffer.rs`** — добавлен `colors::STEP` (оранжевый) в палитру.

### Проверено
- `cargo build --workspace`, `cargo clippy --workspace --all-targets`
  (0 предупреждений), `cargo fmt --all --check`: чисто.
- `cargo test --workspace`: всё проходит.
...
- **`aios-kernel/src/serial.rs`** — `write_bytes` и `SerialWriter::write_str`
  теперь имеют таймаут 0xFFFF в цикле ожидания `while port::inb(COM1+5) & 0x20 == 0`.
  На реальном оборудовании без подключённого последовательного порта
  бит THRE может никогда не стать установленным, что вызывает
  бесконечное зависание на `kprintln!` после `scheduler online`
  (`main.rs:521`). Таймаут позволяет ядру продолжить работу
  без вывода в последовательный порт.
...
административную очередь, идентифицирует контроллер и namespace 1, создаёт одну пару
I/O-очередей и читает логические блоки. Вместе с частью 2a (AHCI) ядро теперь работает
и с SATA-, и с NVMe-дисками при загрузке; USB-HID/xHCI — впереди.

### Добавлено
- **`aios-kernel/src/nvme.rs`** — драйвер NVMe с опросом (без прерываний). Включает
  memory space + bus-mastering PCI, мапит BAR0 (64-битный) через `memory::map_mmio`,
  сбрасывает контроллер (`CC.EN`), программирует `AQA`/`ASQ`/`ACQ`, поднимает его и
  ждёт `CSTS.RDY`. Выполняет `Identify Controller` (модель `MN`, серийник `SN` — ASCII,
  без попарного свопа), `Create I/O Completion/Submission Queue`, `Identify Namespace`
  (NSZE + формат LBA) и `Read` поблочно через PRP1 в один DMA-кадр. Экспортирует
  `Nvme::init`, `drive()` и `read_blocks()` (запись `NvmeDrive`).
- `main.rs` опрашивает класс `01:08` (NVMe) после блока AHCI и выводит модель/серийник/
  объём и магию LBA0.

### Исправлено
- В `AQA` поля упакованы как `ASQS[11:0]` / `ACQS[27:16]`; изначальный сдвиг `<< 12`
  заставлял QEMU читать `ACQS = 0` и выставлять `CSTS.CFS` при запуске
  (`startfail_acqent_sz_zero`).
- Фазовый бит записи завершения — это бит 0 16-битного поля статуса (бит 16 двойного
  слова), а не бит 31; опрос не того бита приводил к таймауту каждой admin-команды.

### Проверено
- `cargo build`/`clippy` (0 предупреждений), `cargo fmt` чисто (цель `x86_64-unknown-none`).
- QEMU (UEFI/OVMF, `-device nvme,serial=AIOSNVME,drive=…` + 16 МиБ `nvme-test.img`):
  `model="QEMU NVMe Ctrl"`, `serial="AIOSNVME"`, `blocks=32768 lba=512 (16 MiB)`,
  `LBA0 = 41 49 4f 53 2d 4e 56 4d | AIOS-NVME-TEST!!`.
- AHCI + NVMe одновременно: оба диска определены и прочитаны за одну загрузку (проверка
  регрессии).
- Базовый запуск (без контроллеров NVMe/AHCI) по-прежнему пишет
  `nvme: no NVMe controller found` и доходит до планировщика.

## v2.36.0 — Bare-metal веха 6 (часть 2a): нативный блочный драйвер AHCI (SATA) (2026-09-18)

Ядро теперь умеет работать с настоящим SATA-контроллером: поднимает порты AHCI,
идентифицирует подключённые диски и читает сектора через DMA. Это «складская» половина
вехи нативных устройств ввода/хранения (USB-HID/xHCI и NVMe — впереди).

### Добавлено
- **`aios-kernel/src/ahci.rs`** — блочный драйвер AHCI (SATA): включает memory space +
  bus-mastering на PCI, сбрасывает HBA и выставляет `AE`, строит на порт кадры
  command-list / FIS-receive / command-table из `alloc_frame`, запускает порт, выполняет
  `IDENTIFY DEVICE` (разбирает ATA-строку модели с попарным свопом байт и объём 28/48 бит)
  и `READ DMA EXT` / `WRITE DMA EXT` через элемент PRDT. Экспортирует `Ahci::init`,
  `drives()` и `read_sectors()` (запись `AhciDrive`).
- **`memory::map_mmio` / `memory::physical_to_virtual`** — BAR устройств мапятся в
  выделенное окно MMIO `0xffffff00_2000_0000` (слот PML4 510), так как HHDM Limine
  не покрывает MMIO; `physical_to_virtual` возвращает HHDM-алиас для DMA-буферов.
- `main.rs` инициализирует хранилище **после** включения прерываний (`sti`), чтобы
  page fault обрабатывался ядром, а не приводил к triple fault.

### Исправлено
- **Удалён `aios-kernel/src/crt.rs`** (поставлялся в v2.35.0). Его сильный `memset` порождал
  циклический PLT: слот GOT релоцировался (`R_X86_64_RELATIVE`) на сам PLT-стаб `memset`,
  из-за чего любой вызов `memset` вне строки превращался в бесконечный `jmp` и вешал
  загрузку. Ядро теперь использует слабые `mem*` из `compiler_builtins`.
- Сигнатура порта (`PxSIG`) читается **после** запуска порта; чтение до запуска в QEMU
  возвращало `0xffffffff`. Порты с сигнатурой ATAPI/SEMB/PM пропускаются.

### Проверено
- `cargo build`/`clippy` (0 предупреждений), `cargo fmt` чисто (цель `x86_64-unknown-none`).
- QEMU (UEFI/OVMF, `-device ich9-ahci` + 16 МиБ `ide-hd` на `ahci.1`): `drives = 1`,
  `model="QEMU HARDDISK"`, `sectors=32768 (16 MiB)`, `LBA0 = 41 49 4f 53 2d 41 48 43 |
  AIOS-AHCI-TEST!!` — совпадает с магией тестового образа.
- Базовый запуск (без AHCI) по-прежнему пишет `ahci: no SATA controller found` и доходит
  до планировщика.
...
оставался чёрным. Причина чёрного экрана — сам *драйвер консоли*: в initramfs нет модулей
VGA-консоли (`vgacon`/`vesafb`/`efifb` не поставляются, `simpledrm` встроен, но не даёт
консоли, пока fbcon не получит буфер кадров), поэтому `tty0` нечему было рисовать, пока
GPU-DRM-драйвер не создавал буфер кадров, — а GPU-модули **не грузились до юзерспейса**,
то есть уже после того, как TUI должен был появиться; к тому же поставленный `aios-init`
вообще не вызывал `bring_userspace()`/`sfs-up.sh`.

### Исправлено
- **Пре-инit загрузчик GPU-модулей (v2.33.3):** запись `/init` в initramfs теперь — это
  шелл-скрипт `aios-loader`, который выполняется *до* любого юзерспейса: он `modprobe`'ит
  все пять x86 GPU-DRM-драйверов (`radeon`, `nouveau`, `gma500_gfx`, `i915`, `amdgpu`),
  чтобы fbcon получил буфер кадров как можно раньше, затем извлекает `dd`-ом оригинальный
  ELF `aios-init` (встроен в скрипт после маркера `#BIN` с запечёнными фиксированной
  ширины `BINOFF:`/`BINSZ:`) и делает `exec` — сохраняя ровно ту же цепочку PID 1
  (`aios-init` → `/system/aios-core` → ядерный TUI).
- **`iso/stage/boot/limine/limine.conf`:** обе записи (Live и Installer) получили
  `loglevel=7`, чтобы отложенный лог ядра (загрузка модулей, передача fbcon) печатался на
  экран, как только регистрируется кадровая консоль, — чёрный экран превращается в видимый
  лог загрузки.

### Заметки о распаковщике initramfs ядра (из расследования)
- Ядро пишет файлы архива до первого `TRAILER!!!`; добавленные после записи игнорируются, а
  удаление/переименование записи `/init` заставляет ядро отвергать весь архив
  (`Unable to mount root fs`). Единственный надёжный способ — **замена содержимого на месте**
  (правка байт-диапазонов существующей записи `/init` с сохранением имени+заголовка+трейлера).
- Тело `/init` — шелл-скрипт со встроенным ELF; извлечение читает сам `/init`
  (`dd skip=$BINOFF count=$BINSZ`) в `/tmp/aios-init`, `chmod 755` и `exec`. Запекание
  должно использовать **поля фиксированной ширины** — замена с переменной длиной цифр
  сдвигает маркер и молча извлекает неверные байты (тогда busybox `ash` парсит ELF как скрипт).

### Верификация
- Прогон патченного initramfs в QEMU даёт `aios-loader: hdr=[ 7f 45 4c 46]` (валидный ELF),
  `aios-loader: chainloading aios-init`, `[aios-init] aios-init: AIOS initramfs init (PID 1)`,
  `[aios-init] aios-init: started block pid …: /system/aios-core` и
  `AIOS TUI mode — starting interactive dashboard` — без паники за 120 с.
- Пересобран `out/aios-live.iso` (369 145 856 B, SHA-256 `FDB69DB8…1452EBA8`), повторно
  установлен MBR Limine (раздел 1 активен, stage 2 @ 0x200), ISO загружен в QEMU с CD:
  SeaBIOS → Limine → ядро с новой командной строкой (`… console=ttyS0 console=tty0
  loglevel=7`) → `Run /init as init process`; fbcon создаётся; GPU-модули грузятся на цель.
- Перепрошит физический Kingston (диск 3): полный SHA-256 флешки == SHA-256 ISO ==
  `FDB69DB8551977CDE4928186F89BFBDA22595A839A24E99C0CBA64DC1452EBA8` (побайтово),
  MBR-sig 55AA, stage 2 @ 0x200, head/tail совпадают.

## v2.33.2 — Фикс загрузки на реальном железе: только serial-консоль оставляла экран ноутбука чёрным (2026-09-16)

Загрузочный носитель без проблем грузился в QEMU (наш харнесс писал именно serial-лог),
но первый физический тест на ноутбуке пользователя с чистым BIOS дал чёрный экран сразу
после передачи управления ядру от «vmlinuz / initramfs». Причина: в обеих записях Limine
стоял `console=ttyS0` как **единственная** консоль, поэтому `/dev/console` — в него
`aios-init` (`setup_console()` делает dup2 на фд 0/1/2), TUI ядра, установщик и аварийный
шелл выводят всё — смотрел в несуществующий COM-порт. Система фактически грузилась и
работала, но невидимо.

### Исправлено
- **`iso/stage/boot/limine/limine.conf`:** обе записи (Live + Installer) теперь используют
  `console=ttyS0 console=tty0` (tty0 последней → `/dev/console` = VGA). Printk ядра идёт в
  обе консоли; TUI AIOS Live, установщик и аварийный шелл видны на экране ноутбука.
- **`live/aios-install`:** `grub.cfg`, записываемый в установленную систему, теперь
  использует `console=tty0` вместо serial-only `console=ttyS0` — установленный AIOS не
  получит тот же чёрный экран.

### Проверено
- Пересобран `out/aios-live-v2.iso` (368 777 216 Б) и заново записан MBR Limine в ISO
  (раздел 1 активен, stage 2 @ 0x200).
- QEMU: новый ISO при загрузке raw-диском регистрирует обе консоли
  (`printk: legacy console [tty0] enabled`, `[ttyS0] enabled`) и доходит до
  `Run /init as init process`; прямой запуск ядра подтверждает
  `aios-init: started block pid …: /system/aios-core` с последующим
  `AIOS TUI mode — starting interactive dashboard`; screendump VGA через монитор QEMU
  показывает живой (не чёрный) экран.
- Флешка Kingston (Диск 3) перезаписана: SHA-256 флешки == SHA-256 ISO ==
  A73DAD4F…9AA4F9 (байт-в-байт), сигнатура MBR 55AA, stage 2 @ 0x200 подтверждены.

## v2.33.1 — Надёжность установщика: verbose-режим, журнал установки на целевом диске, защита от повторной установки (2026-09-15)

Фоллоу-апы по надёжности из ревью v2.30.0: установщик теперь пишет журнал установки
на целевой диск, поддерживает verbose-режим, фиксирует фактическое размещение
корневого раздела (остаток диска), защищает предыдущую установку от случайного
затирания (интерактивно) и повторно проверяет безлюдные (`--auto`) установки
end-to-end.

### Добавлено
- **`live/aios-install` — журнал установки, сохраняемый на целевом диске.** Каждый
  шаг с меткой времени дописывается в `/aios-install.log` в корневом разделе
  (`AIOS Installer start`, `root size: <байт> (rest of disk)`, `system copied`,
  `BIOS/EFI grub rc=...`, `grub.cfg written`, `AIOS INSTALL COMPLETE`), дублируется в
  консоль и печатается целиком перед баннером успеха. При физической установке любая
  ошибка теперь читается с целевого диска.
- **Переключатель verbose.** `aios.verbose` в командной строке ядра (или
  `--verbose`/`-v` при прямом запуске `aios-install`) дополнительно выводит финальную
  таблицу разделов (`sfdisk --dump`) перед копированием.
- **Защита от повторной установки.** Если на `${DEV}3` уже стоит метка AIOS
  (предыдущая установка), интерактивный запуск требует ввести
  `Type YES to FORCE reinstall`; безлюдный путь `aios.yes` пропускает лишний запрос и
  продолжает (для QEMU/скриптового приёмочного цикла).
- **Корневой раздел полного размера.** Root-раздел по дизайну занимает остаток диска
  (`,,` в sfdisk); фактический размер теперь фиксируется в журнале (проверено:
  1607467008 байт на 2 ГиБ QEMU-диске).

### Изменено
- **Инъекция cmdline в `build_final.py`:** `aios.yes` теперь также выставляет
  `AUTO=1`, а `aios.verbose` — `VERBOSE=1` (строка `AIOS-dbg` печатает оба).
- **Пересобран загрузочный ISO** (двухпунктовый Limine, `rdinit=/installer`) с новым
  установщиком, записан raw-записью на физическую флешку и сверен побайтово
  (head/tail).

### Проверено
- QEMU-установка безлюдно (`rdinit=/installer aios.target=sda aios.yes
  console=ttyS0 panic=5`, 2 ГиБ vdisk, 2048 МиБ RAM): журнал со всеми шагами и
  `AIOS INSTALL COMPLETE`, BIOS grub rc=0, EFI grub rc=0; установленный диск
  грузится в `AIOS TUI mode - starting interactive dashboard`.
- Прогон `aios.verbose` показывает `VERBOSE='1'` и выводит GPT-таблицу `label: gpt`.
- Флешка: WROTE 368 777 216 байт, `RESULT: OK` (head+tail совпали).
...
### Исправлено
- **Длинные строки больше не обрезаются** в едином TUI `aios`: `draw_shell_tab` и `draw_ai_output` (`aios/src/tui/ui.rs`) теперь переносят вывод по ширине панели вместо обрезания по правому краю. `wrap_line` переписан со счётчика символов на **splitter по ширине отображения** через `unicode-width`, поэтому многобайтовый текст (кириллица, CJK, эмодзи) переносится по правильной ширине терминала, а комбинируемые знаки не отрываются от базового символа. В Cargo.toml пакета `aios` добавлен `unicode-width = "0.2"` (уже был в дереве через ratatui/crossterm; новых записей в lock нет).

### Верификация
- Новый юнит-тест `wrap_line_wraps_at_display_width` покрывает ASCII-чанки, кириллицу (ширина 1), CJK (ширина 2 на символ), комбинируемые знаки и краевой случай `width == 0`; все 5 `render_smoke_tests` проходят, `cargo clippy -p aios` — 0 предупреждений, `cargo fmt --check` чисто.
...
### Добавлено
- **Диалог второго шанса** (`aios-studio`): модальное окно (`#confirm-overlay`) с иконкой-предупреждением, текстом сообщения и кнопками Cancel / Proceed (Enter подтверждает, Esc отменяет) теперь показывается перед любым потенциально деструктивным действием:
  - **Командная палитра** — промпты, содержащие `kill / terminate / unload / stop / remove / delete / compact / free`, требуют подтверждения до отправки интента.
  - **Кнопка "Stop" в таблице блоков** — выгрузка теперь идёт через `unloadBlock(name)` с именованным подтверждением.
  - **Быстрое действие "Compact Memory"** дашборда — идёт через `compactMemory()` с предупреждением о системном применении.
  - **"Run" в билдере воркфлоу** — если воркфлоу содержит шаги `kill`, `unload_block` или `compact`, перед выполнением выводится список всех деструктивных шагов (метка + промпт) и один запрос на подтверждение.
- Раньше все три точки входа (палитра, быстрые действия, Stop блока) исполняли деструктивные операции одним кликом; у билдера воркфлоу защиты не было вовсе.

### Верификация
- `node --check aios-studio/app.js` — проходит; в этом релизе правок Rust нет.
...
### Исправлено
- **Ядерный TUI больше не «сваливается в кучу» при переключении вкладок на низком окне.** Вкладка System & HW считает позицию шкалы RAM как `chunk.y + chunk.height - 3`; на малых высотах ratatui отдаёт `chunk.height == 0` (констрейнты 9+8+10 превышают панель), поэтому шкала прыгала в строку `0` и рисовалась **поверх полосы статуса** — тот самый артефакт «всё в кучу». Позиция шкалы теперь ограничена `saturating_sub` и рисуется только при достаточной ширине — она больше не может покинуть левую панель.
- **Полоса статуса перестала обрезаться по словам.** Старая компоновка из фиксированных сегментов сжималась пропорционально (констрейнты `Length` — жёсткие равенства; `Flex::Start` влияет только на спейсеры), обрезая заголовки до `Network & Stor` / `Blocks & Sv`. `draw_status_bar()` теперь рисует один `Paragraph` из `Line` со стилизованными `Span` (идиома Far/MC) — он обрезается только по правому краю терминала.
- **Упавший фоновый воркер бриджа больше не может убить TUI / оставить терминал в raw-режиме.** Паника хендлера при удержании `registry`/`scheduler` отравляла эти мьютексы, следующий `.lock().unwrap()` при отрисовке паниковал, и терминал оставался в raw-режиме. Добавлен отказоустойчивый к poisoning `locked()` (`app_state.rs`), на него переведены все locks путей отрисовки и хендлеров в `ui.rs`/`mod.rs`, а цикл `run()` обёрнут в `std::panic::catch_unwind`, который **всегда** восстанавливает терминал (выключение raw-режима, выход из alternate screen, возврат курсора) и возвращает `AIOS TUI crashed: …`.
- **`test_e2e_bridge_http_endpoints` снова зелёный.** Вызовы эндпоинтов написаны до введения web-авторизации в v2.30.0: `/api/v1/system/status`, `/workflow`, `/metrics`, `/intent` теперь защищены, а тест не слал токен. Тест теперь регистрирует пользователя (а если он уже есть на диске — логинится) и передаёт `Bearer`-токен. Остальное уже было зелёным; сюита воркспейса проходит полностью.
- **Hardware Inspector на вкладке 1 (System & HW) больше не сдвигается при каждом посещении.** Лента hot-plug тостов резервировала `min(toasts,4)+1` строк, вырастая с 1 до 5 строк на каждый новый тост и перетряхивая всю таблицу устройств под ней — при каждом переключении появлялась новая строка и контент прыгал. Лента теперь резервирует **фиксированный** блок из 5 строк, как только появляются тосты (показываются последние 4), так что таблица устройств стоит на месте, а новые тосты замениют старые на тех же позициях вместо сдвига компоновки.
- **Больше нет повторного «фантомного» провижининга на Windows.** На тестовой машине PnP-запись `USB 0000:0000 (unknown)` (устройство без какого-либо идентификатора) «флапала» каждые несколько секунд, порождая `WM_DEVICECHANGE` → полный re-detect → диф отпечатков → попытку провижининга → тост, и так по кругу — то самое «при каждом открытии вкладки 1 появляется новая строка». Три исправления: (1) `HardwareProfile::detect_usb` теперь отбрасывает записи `0000:0000` как артефакты энумерации (новый helper `is_identifiable_usb`, покрыт unit-тестами), и они не доходят до дифа отпечатков; (2) нативный hot-plug push коалесцируется в `HotplugMonitor` — залп событий вызывает не более одного скана за `poll_ms`, а не по одному на событие; (3) INFO-строки `HAL: Detected …` / `HAL: NVIDIA GPU detected` выводятся один раз за процесс вместо каждого rescan.
- **Оверлей справки F1 наконец рисуется в ядерном TUI.** `show_help` переключался по `F1`/`?`, но нигде не отрисовывался — нажатие F1 не давало никакого видимого результата, несмотря на метку «Help» в F-баре и описание оверлея в INTERFACE/ARCHITECTURE. `draw()` теперь рисует полноэкранный непрозрачный блок `AIOS Help` (глобальные + контекстные клавиши; фон затирается, чтобы текст не сливался). Поведение «удержания»: пока F1 зажат, оверлей остаётся открытым; закрывается по `F1`/`?`/`Esc`/`h`. Покрыто тестом `help_overlay_renders_on_all_sizes` (40×12…120×30).
- **Ручной re-probe по `F10` привязан в ядерном TUI.** Текст в инспекторе вкладки System & HW и доки INTERFACE уже обещали «press F10 to re-probe», но обработчика `F10` не было (метка в F-баре `Quit`, а выход и так на `q`). `F10` теперь вызывает `refresh_hw()` (полный `HardwareProfile::detect()` + rescan движка) на любой вкладке; метка в F-баре изменена `10Quit` → `10Rescan`, и render-тест проверяет её.
- Применён `cargo fmt --all` по `aios-bridge` (косметика, без изменения семантики).

### Добавлено
- `aios/src/tui/ui.rs` `#[cfg(test)] mod render_smoke_tests` на `TestBackend` (в ratatui 0.29 доступен всегда — фича-гейт не нужен), с `make_app()` (safe-mode `TuiApp::new`, `HwProfile`/`BridgeContext` собираются с нуля) и тремя тестами:
  - `all_tabs_render_without_panic_and_with_chrome` — все 7 вкладок × размеры 120x30 → 40x12; проверяет, что полоса статуса (версия/заголовок вкладки) живёт и ничего не паникует.
  - `net_and_shell_prompts_render_in_both_editing_modes` — переключение приглашений `net>`/`AIOS>`.
  - `poisoned_locks_do_not_break_rendering` — отравляет `registry` паникой в потоке, после чего рисует все вкладки (до `locked()` это паниковало).
- Хелпер `locked()`: `pub fn locked<'a, T>(m: &'a Mutex<T>) -> MutexGuard<'a, T>`, превращающий poisoning мьютекса в восстановление через `into_inner()`.

### Верификация
- `cargo test --workspace`: все 22 тест-бинарника зелёные (~500+ тестов), 3 подряд зелёных прогона e2e-теста моста.
- `cargo clippy --workspace`: 0 предупреждений. `cargo fmt --all --check`: чисто.

Файлы: `aios/src/tui/{ui,mod,app_state}.rs`, `tests/e2e_pipeline_test.rs`, `docs/{CHANGELOG,BUGS,INTERFACE,ARCHITECTURE}{,.ru}.md`.
...
### Что добавлено
- Полный аудит проекта (`docs/AUDIT.ru.md` + `docs/AUDIT.md`): сборка OK, `cargo clippy --workspace --all-targets` — 0 предупреждений, `cargo fmt --check` — чисто, `cargo test --workspace` — **1338 пройдено / 0 упало** в 91 наборе; срез кодовой базы: 244 файла `.rs`, ~59 400 строк, таблица файлов/строк/тестов по крейтам.
- Новая схема программы с картой функций (`docs/SCHEME.ru.md` + `docs/SCHEME.md`): карта слоёв всех крейтов, порядок загрузки бинарника `aios`, диаграмма потока IPC, таблицы модулей/функций по крейтам (фундамент, подсистемы ядра, службы, веб-стек, слой ИИ), таблица REST-эндпоинтов `aios-bridge`, статус вех bare-metal-микроядра, карта покрытия интеграционными тестами.
- Исправлен баг: `tests/stress_test.rs::test_stress_rt_scheduler_500` «мигал» на загруженной машине ("RT scheduling took 2.0974127s (>2s)") — жёсткий порог 2 с по wall-clock нарушал правило двойных порогов debug/release; теперь 5 с при `cfg!(debug_assertions)`, 2 с в release. Повторный прогон набора зелёный (11/11).
- Гигиена дорожной карты: Phase 46 (`aios-autohal`) отмечена выполненной в `TODO.md`/`TODO.ru.md` — все подпункты вышли в v2.22.0–v2.25.0, но родительская галочка осталась открытой.
- Файлы: `tests/stress_test.rs`, `docs/{AUDIT,SCHEME}{,.ru}.md` (новые), `docs/{CHANGELOG,BUGS,TODO}{,.ru}.md`, `docs/ARCHITECTURE{,.ru}.md` (ссылки see-also).

## v2.28.0 — aios-kernel, веха 2: paging + куча ядра (2026-08-15)

### Что добавлено
- `aios-kernel` получил собственное ядро paging: обход таблиц страниц, который читает активные PML4/PDPT/PD/PT через карту физической памяти загрузчика (`memory::translate`, большие страницы поддерживаются), плюс `memory::map_page`/`unmap_page`, выделяющие кадры для таблиц по требованию и сбрасывающие TLB через `invlpg`.
- Новый модуль `memory`: `init` строит bump-аллокатор кадров по `Usable`-регионам из boot info (постраничное выравнивание, до 16 регионов); `alloc_frame` выдаёт отдельные 4 КиБ кадры; `selftest` мапит тестовую страницу по `0xFFFF880000000000`, пишет/читает 64-битный паттерн, проверяет `translate`, затем делает unmap и проверяет, что страница исчезла.
- Новый модуль `heap`: куча ядра 2 МиБ по `HEAP_START = 0xFFFF840000000000`, замаплена страница за страницей через `memory::map_page`; `#[global_allocator]` free-list аллокатор (first-fit со сплитом, коалесценцией с головным блоком и spin-lock) позволяет использовать типы из `alloc`-крейта (`Box`, `Vec`, `String`) внутри ядра. `heap::init_heap` создаёт начальный свободный блок, `heap::test_heap` выделяет `Vec<u64>` на 1000 элементов, `String`, `Box<f64>` и стресс-цикл из 200 коротких строк.
- `kernel_main` теперь инициализирует memory manager и кучу между вехами 0 и 1; в лог попадают `memory manager init, usable regions = N`, `paging selftest OK.`, результаты кучи и `Milestone 2: paging + kernel heap online.`
- Проверено под QEMU: `[serial] translate vga=0xNone kernel=0xSome(1013150) heap_before=0xNone`, `[serial] paging selftest OK.`, `[serial] heap: Vec<u64> 1000 elems, sum=999000`, `[serial] heap: String 'heap string ok'`, `[serial] heap: Box<f64> 3.25`, `[serial] heap: stress 200 strings len_sum=5100, final Vec sum=1498500`, после чего веха 1 по-прежнему загружается с `tick 1s/2s/...`.

### Исправленные баги
- Free-list аллокатор кучи возвращал полезную нагрузку выделенного блока, не записав в заголовок фактический размер выделения — в заголовке оставался полный (досплитовый) размер, поэтому `dealloc` вычислял коалесценцию с неверным размером и портил free list (впервые проявилось как мусорная сумма `Vec`: `18446198715943183352` вместо `999000`). Исправлено записью точного размера (`needed`) в `(*block).size` сразу после решения о сплите.
- Файлы: `aios-kernel/src/{main,memory,heap}.rs`, `docs/*`.



## v2.27.0 — aios-kernel, веха 1: прерывания работают (GDT/TSS, IDT, PIC, PIT, клавиатура) (2026-08-15)

### Что добавлено
- `aios-kernel` получил полный стек прерываний x86_64: GDT на 7 записей с 64-битным TSS (double-fault IST), IDT на 256 записей, привязанную к ассемблерным стабам векторов, ремап 8259 PIC на `0x20`/`0x28`, таймер PIT на 100 Гц и обработку IRQ клавиатуры PS/2.
- Новые модули: `gdt` (GDT + TSS, `lgdt`/`reload_segments`/`ltr`), `idt` (256 гейтов из `aios_handler_table`), `interrupts` (диспетчер `aios_handle_interrupt`, `init_pic`, `init_pit`, `pic_eoi`, таблица скан-кодов), `port` (`outb`/`inb`/`io_wait`).
- `build.rs` генерирует `irq_stubs.S`: 256 стабов на вектор плюс `aios_interrupt_common` (сохранение/восстановление кадра, вызов C-обработчика). Таблица хендлеров лежит в `.data.rel.ro` (с `.hidden`), стабы векторов и общий путь — в `.text`.
- `kernel_main` теперь устанавливает GDT/TSS, IDT, PIC и PIT, выполняет `sti` и входит в idle-цикл, который печатает `tick Ns` каждую секунду и декодирует скан-коды клавиатуры (`key 'x' (0xNN)`).
- Смоук-тест вехи 1 проходит под QEMU: `[serial] Milestone 1: interrupts online.`, затем `tick 1s/2s/...`; введённые через монитор QEMU (`sendkey h`) клавиши печатаются как `[serial] key 'h' (0x23)`.

### Исправленные баги
- `Descriptor { limit, base }` в `gdt.rs`/`idt.rs` был `#[repr(C)]`; поле `u64` базы оказывалось на смещении 8, а `lgdt`/`lidt` читают 10-байтовый дескриптор как limit на 0 и базу на 2, поэтому база GDTR/IDTR получалась как `padding + первые два байта базы` (например, `0xd960` вместо `0x1000001d960`). Итог: любая загрузка сегментов/IDT через новую GDT фолтила (`#GP` на `lretq`/`mov ds`, `#DF` при доставке по IDT, тройной фолт). Исправлено `#[repr(C, packed)]` и `addr_of_mut!` для поля дескриптора.
- Блоки `global_asm!` склеиваются в один ассемблерный поток; директива `.section .data.rel.ro` из сгенерированного `irq_stubs.S` «протекала» в следующий `global_asm!`, помещая код `aios_reload_segments` в секцию данных. Исправлено явным `.text` в начале ассемблера модуля `gdt`.
- Дальний возврат для перезагрузки CS (`push 0x08; ... retfq`) был не нужен — загрузчик уже запускает ядро с CS=`0x08`/SS=`0x10`, которые после `lgdt` резолвятся в собственные дескрипторы ядра — и он фолтил, пока GDT была сломана. Заменён на простую перезагрузку DS/ES/FS/GS в `0x10`.
- `aios_handler_table` изначально лежала в `.rodata`; абсолютные релокации там не линковались. Перенесена в `.data.rel.ro` с `.hidden`, чтобы релокации применялись загрузчиком при загрузке (проверено: `table[0]=0x10000017500` в рантайме).
- Файлы: `aios-kernel/build.rs`, `aios-kernel/src/{main,gdt,idt,interrupts,port}.rs`, `docs/*`.
