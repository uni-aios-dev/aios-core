...
### `aios-wasm`: уточнена семантика дедлайна эпохи
- **BUG-038 (НИЗКИЙ)** — в wasmtime дедлайн эпохи по умолчанию равен 0, что мгновенно прерывает выполнение при включённом прерывании по эпохе. `set_epoch_deadline(1)` сохраняет выполнение безопасным (счётчик эпохи движка начинается с 0) и активирует тайм-аут для host-тикера; конфигурация песочницы и создание store теперь это документируют. `timeout_ms` — подсказка до добавления тикера.
...
- 15+ новых тестов на каждый фикс.
